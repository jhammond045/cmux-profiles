#!/usr/bin/env bash
# cmux-profiles installer: link the Spoon into ~/.hammerspoon/Spoons, seed a config,
...
HS="$HOME/.hammerspoon"
SPOONS="$HS/Spoons"
CFG="$HOME/.config/cmux-profiles"

mkdir -p "$SPOONS" "$CFG"

ln -sfn "$SRC/CmuxProfiles.spoon" "$SPOONS/CmuxProfiles.spoon"
echo "linked  $SPOONS/CmuxProfiles.spoon -> $SRC/CmuxProfiles.spoon"
...
LOADER='hs.loadSpoon("CmuxProfiles"):start()'
if [[ -f "$HS/init.lua" ]] && grep -qF 'loadSpoon("CmuxProfiles")' "$HS/init.lua"; then
